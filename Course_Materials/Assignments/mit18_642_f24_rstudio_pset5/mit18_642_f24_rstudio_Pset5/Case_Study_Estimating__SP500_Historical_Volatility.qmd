---
title: "Case Study: Estimating Historical Volatility of SP500 Stock  Index"
...
First, import prices of S\&P500 Index and SP500 Stock using the function tq\_get() from the library tidyquant.
...
SP500<-tq_get("^GSPC", to="2024-10-31")

symbolname0<-"SP500"

```

The object SP500 consists of daily bar data, collected from \url{finance.yahoo.com}
...
ggplot(data.frame(SP500), aes(x=date, y=close)) + geom_line() +
...
```{r}
#help("volatility")
SP500.vol.close<-volatility( SP500[,3:6], n=21, calc="close", N=252)
SP500.vol.close.0<- xts(x=SP500.vol.close, order.by=SP500$date)
autoplot(SP500.vol.close.0)
...
```{r}
#help("volatility")
SP500.vol.parkinson<-volatility( SP500[,3:6], n=21, calc="parkinson", N=252)
SP500.vol.parkinson.0<- xts(x=SP500.vol.parkinson, order.by=SP500$date)
autoplot(SP500.vol.parkinson.0)
```
...
```{r}
#help("volatility")
SP500.vol.garman.klass<-volatility( SP500[,3:6], n=21, calc="garman.klass", N=252)
SP500.vol.garman.klass.0<- xts(x=SP500.vol.garman.klass, order.by=SP500$date)
autoplot(SP500.vol.garman.klass.0)
```
...
```{r}
#help("volatility")
SP500.vol.rogers.satchell<-volatility( SP500[,3:6], n=21, calc="rogers.satchell", N=252)
SP500.vol.rogers.satchell.0<- xts(x=SP500.vol.rogers.satchell, order.by=SP500$date)
autoplot(SP500.vol.rogers.satchell.0)
```
...
```{r}
#help("volatility")
SP500.vol.gk.yz<-volatility( SP500[,3:6], n=21, calc="gk.yz", N=252)
SP500.vol.gk.yz.0<- xts(x=SP500.vol.gk.yz, order.by=SP500$date)
autoplot(SP500.vol.gk.yz.0)
```
...
```{r}
#help("volatility")
SP500.vol.yang.zhang<-volatility( SP500[,3:6], n=21, calc="yang.zhang", N=252)
SP500.vol.yang.zhang.0<- xts(x=SP500.vol.yang.zhang, order.by=SP500$date)
autoplot(SP500.vol.yang.zhang.0)
```
...
```{r}
SP500.vols<- merge.xts(
  SP500.vol.close.0,
  SP500.vol.parkinson.0,
  SP500.vol.garman.klass.0,
  SP500.vol.rogers.satchell.0,
  SP500.vol.gk.yz.0,
  SP500.vol.yang.zhang.0
)

names(SP500.vols)<-c("Close","Parkinson","GK","RS","GK.YZ","YZ")
...
```r 
autoplot(SP500.vols, facets =FALSE)
```

In RStudio cloud it did not. We now construct the figure directly.

```{r}
SP500.vols0<- data.frame(SP500.vols)
names(SP500.vols0)
SP500.vols0$date<- as.Date(time(SP500.vols))
SP500.vols00<- pivot_longer(SP500.vols0,cols=c(1:6), names_to="model")
ggplot(SP500.vols00, aes(x=date, y=value, col=model)) + geom_line()
```
...
```{r}
volmat0<-window(SP500.vols, start=as.Date("2023-01-01"))
...
```{r}
gg0<-ggplot(data.frame(SP500.vols), aes(x=Close, y=YZ)) + geom_point(alpha=0.1)+
